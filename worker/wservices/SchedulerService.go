...
func (scheduler *Scheduler) handleJobEvent(jobEvent *tools.JobEvent) {
	...
	case constants.JOB_EVENT_DELETE: // 删除任务事件
...
// 尝试执行任务
func (scheduler *Scheduler) TryStartJob(jobPlan *tools.JobSchedulePlan) {
	// 调度 和 执行 是2件事情
	var (
		jobExecuteInfo *tools.JobExecuteInfo
		jobExecuting   bool
	...
	// 构建执行状态信息
	jobExecuteInfo = tools.BuildJobExecuteInfo(jobPlan)
...
// 处理任务结果
func (scheduler *Scheduler) handleJobResult(result *tools.JobExecuteResult) {
	//var (
	//	jobLog *tools.JobLog
	//)
	// 删除执行状态
	...
	// 生成执行日志
	//if result.Err.Error() != constants.ERR_LOCK_ALREADY_REQUIRED {
	//	jobLog = &tools.JobLog{
...
// 推送任务变化事件
func (scheduler *Scheduler) PushJobEvent(jobEvent *tools.JobEvent) {
	scheduler.jobEventChan <- jobEvent
}

// 回传任务执行结果
func (scheduler *Scheduler) PushJobResult(jobResult *tools.JobExecuteResult) {
